...
formLogin.addEventListener('submit', async (e) => {
    ...
    console.log('🔐 Tentando fazer login com Discord:', nomeDiscord);
    ...
    try {
        ...
        if (verificarAdmin(nomeDiscord, senha)) {
            console.log('👑 Login de ADMINISTRADOR detectado!');
            
            // Salvar como admin na sessão
            localStorage.setItem('omega_usuario_logado', JSON.stringify({
                nomeDiscord: nomeDiscord,
                isAdmin: true
            }));
        ...
        // Se não é admin, tentar login normal
        const resultado = fazerLogin(nomeDiscord, senha);
        
        if (resultado.success) {
            // Login bem-sucedido - redirecionar para dashboard
            console.log('✅ Login realizado!', resultado.jogador);
...
function mostrarDadosJogador(jogador) {
    ...
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log(`Discord: ${jogador.nomeDiscord}`);
    console.log(`Roblox: ${jogador.nomeRoblox}`);
    console.log(`Nível: ${jogador.lvl}`);
    console.log(`Posição Principal: ${jogador.posicaoPrincipal}`);
    console.log(`Posições Jogáveis: ${jogador.posicoesJogaveis.join(', ')}`);
    console.log(`Descrição: ${jogador.descricaoJogo}`);
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    ...
    card.innerHTML = `
        <h3 style="color: #60a5fa; margin-bottom: 1rem;">🎮 Perfil do Jogador</h3>
        <p><strong>Discord:</strong> ${jogador.nomeDiscord}</p>
        <p><strong>Roblox:</strong> ${jogador.nomeRoblox}</p>
        <p><strong>Nível:</strong> <span style="color: #a855f7;">${jogador.lvl}</span></p>
        <p><strong>Posição:</strong> ${jogador.posicaoPrincipal}</p>
        <p><strong>Posições:</strong> ${jogador.posicoesJogaveis.join(', ')}</p>
        <button onclick="logout(); this.parentElement.remove();" style="
            margin-top: 1rem;
            padding: 0.5rem 1rem;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Não mostrar nada se houver sessão - apenas logar no console
    const usuario = getUsuarioLogado();
    
    if (usuario) {
        console.log('✅ Sessão ativa detectada para:', usuario.nomeDiscord || 'Admin');
    } else {
